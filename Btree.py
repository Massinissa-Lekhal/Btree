...
class Btree : 
    ...
    def coverage(self,k) : 
        """
        verifier les proprieté de l'arbre d'ordre k
        Parametres : 
        - k :
        """
        if (len(self.root.childs) == 0) : 
            ...
            condition1 = len(self.root.keys)>0 and len(self.root.keys)<=self.k
            condition2 = len(self.root.childs)>=k//2 and len(self.root.childs)<=k+1
